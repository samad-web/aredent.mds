...
const COLS = "stream,year,round,rank,college,course,quota,category,state,is_pwbd";
const mapRow = (r) => ({
  stream: r.stream || "PG",
  year: r.year, round: r.round, rank: r.rank,
  college: r.college, course: r.course,
  quota: r.quota, category: r.category ?? undefined,
  state: r.state ?? undefined, isPwBD: r.is_pwbd,
});

// Fetch allotment rows for one stream ("PG" | "MDS"). Filtering server-side
// keeps each view light (the MDS set is ~5k rows vs ~93k for PG). Pass
// stream=null to fetch every stream at once.
export async function fetchAllotmentRecords({ stream = "PG", pageSize = 1000, concurrency = 8 } = {}) {
  if (!supabase) throw new Error("Supabase not configured");

  const withStream = (q) => (stream ? q.eq("stream", stream) : q);
  ...
  async function worker() {
    for (let p = nextPage++; p < pages; p = nextPage++) {
      ...
      const { data, error } = await withStream(
        supabase
          .from("allotment_records")
          .select(COLS)
          .order("id", { ascending: true })
